    ...
    class ActionHandler{
        ...
        void continue_action(Action &act, game_state &state){
            ...
            // Else, we continue the action: updating the game state variables
                ...
                switch (act.type){
                    case Action::THRUST:
                        // robust_thruster decides/rotates/fires every tick this
                        // action is active, and sets state.accel appropriately
                        // (thrust vector once actually firing, otherwise leaves
                        // the gravity-only default that run_actions() already set).
                        robust_thruster(act, state);
                        break;
                    case Action::ROTATE:
